...
def random_translate(image, target):
    ...
    if random.random() > 0.5:
        ...
        target[:, 0] -= 1.0 * c / image_width
        target[:, 1] -= 1.0 * f / image_height
        return image, target
    else:
        return image, target
...
def pad_crop(image, target):
    ...
    border_img = np.zeros(
        (image_width + border_size[0] + border_size[1], image_height + border_size[2] + border_size[3], 3)
    ).astype(np.uint8)

    border_img[
        border_size[0] : border_size[0] + image_height, border_size[2] : border_size[2] + image_width, :
    ] = image_np

    image_pil = Image.fromarray(border_img.astype("uint8"), "RGB")
    image_pil = image_pil.resize((image_height, image_width))
    target = (target * np.array([image_height, image_width]) + np.array([border_size[2], border_size[0]])) / np.array(
        [border_img.shape[1], border_img.shape[0]]
    )
...
def AUGHIH(image, keypoints):
    image = Image.fromarray(image)
    keypoints = keypoints / 255
    img, target = random_translate(image, keypoints)
    # img, target = random_flip(img, target, flip_points(data_name))
    img, target = random_rotate(img, target, angle_max=30)
    img = random_blur(img)
    img = random_occlusion(img)
    img, target = pad_crop(img, target)
    return {"image": np.array(img), "keypoints": target * 255}
