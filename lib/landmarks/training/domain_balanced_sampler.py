    ...
    *,
    ...
    for item in value.split(","):
        ...
        try:
            ...
            raise ValueError(f"target for {target_key!r} has invalid value {raw_value!r}") from exc
        if not math.isfinite(amount) or amount < 0:
            raise ValueError(f"target for {target_key!r} must be a finite non-negative value, got {raw_value!r}")
...
def sample_bucket(sample: T.Mapping[str, T.Any]) -> str:
    ...
    bucket = canonical_bucket(metadata.get("hard_negative_bucket") or sample.get("condition"))
...
def sample_dataset(sample: T.Mapping[str, T.Any]) -> str:
    metadata = _metadata(sample)
    return normalize_dataset(sample.get("dataset") or metadata.get("dataset")) or "unknown"
...
class DomainBalancedBatchSampler(Sampler[list[int]]):
    ...
    def __init__(
        ...
        *,
        ...
        self.bucket_targets = _normalize_targets(bucket_targets or DEFAULT_BUCKET_TARGETS, kind="bucket")
    ...
    def _build_groups(self, samples: T.Sequence[T.Mapping[str, T.Any]]) -> dict[tuple[str, str, str], list[int]]:
    ...
    def __iter__(self):
        ...
        for batch_index in range(global_batches):
            ...
            for slot, bucket in enumerate(bucket_plan):
                ...
                index, fallback = self._draw_index(pools, rng, bucket=bucket, dataset=dataset, schema=schema)
                batch.append(index)
                if batch_index % self.world_size == self.rank:
                    diagnostics["fallback_counts"][fallback] = int(diagnostics["fallback_counts"].get(fallback, 0)) + 1
    ...
    def _empty_diagnostics(self) -> dict[str, T.Any]:
        return {
            ...
            "missing_targets": {
                "bucket": sorted(_missing_targets(self.bucket_targets, {key[0] for key in self._groups})),
                "dataset": sorted(_missing_targets(self.dataset_targets, {key[1] for key in self._groups})),
                "schema": sorted(_missing_targets(self.schema_targets, {key[2] for key in self._groups})),
            },
            "rank": int(self.rank),
    ...
    def _increment_mix(self, diagnostics: dict[str, T.Any], sample: T.Mapping[str, T.Any]) -> None:
        for key, label in (
...
def _normalize_targets(targets: T.Mapping[str, float], *, kind: str) -> dict[str, float]:
    return {normalize_target_key(key, kind=kind): float(value) for key, value in targets.items()}
...
def _missing_targets(targets: T.Mapping[str, float], observed: set[str]) -> set[str]:
    return {key for key, value in targets.items() if key != "*" and float(value) > 0.0 and key not in observed}
