...
class AddCoordsTh(nn.Module):
    ...
    def forward(self, input_tensor, heatmap=None):
        ...
        xx_range = (
            torch.arange(self.x_dim, dtype=torch.int32).unsqueeze(0).to(input_tensor)
        )
        ...
        yy_range = (
            torch.arange(self.y_dim, dtype=torch.int32).unsqueeze(0).to(input_tensor)
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if with_boundary and not first_one:
            in_channels += 2
        self.conv = nn.Conv2d(
            in_channels=in_channels, out_channels=out_channels, *args, **kwargs
        )
...
class AddCoords(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        yy_channel = (
            torch.arange(y_dim).repeat(1, x_dim, 1).transpose(1, 2).to(input_tensor)
        )
