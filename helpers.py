...
class bottleneck_IR_SE2(Module):
    def __init__(self, in_channel, depth, stride=1):
        super(bottleneck_IR_SE2, self).__init__()
        if in_channel == depth:
            self.shortcut_layer = MaxPool2d(1, stride)
        else:
        ...
        self.merge = nn.Sequential(
            WSConv2d(depth * 2, depth, 3, 1, 1, bias=False),
    ...
    def forward(self, x):
        shortcut = self.shortcut_layer(x)
        res = self.res_layer(x)
        r = torch.cat([shortcut, res], dim=1)
        r = self.merge(r)
        return r


if __name__ == "__main__":
    b = bottleneck_IR_SE2(3, 64, 1)
